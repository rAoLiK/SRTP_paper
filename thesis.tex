...
\begin{figure}[htbp]
    ...
    \caption{固定宽度改进 Booth 乘法器中保留部分与截断部分的划分框架}
...
\begin{figure}[htbp]
    ...
    \caption{TOSAM 与其他近似乘法器的延迟-能量对比}
...
\begin{figure}[htbp]
    ...
    \caption{近似乘法器误差特征与神经网络应用精度退化关系图}
...
\begin{figure}[htbp]
    ...
    \caption{Transformer 应用中自注意力模块的延迟占比分解}
...
\begin{figure}[htbp]
    ...
    \caption{Transformer 加速器中数据搬移数量与能耗分解图}
...
第二，构建算子级与应用级的误差刻画与映射流程，进行全遍历误差统计、误差特征建模以及基
于量化和再训练的鲁棒性评估；
...
\subsection{典型近似乘法器架构剖析}
本节对三种典型近似乘法器架构进行分析，从部分积生成、压缩树结构、关键路径与误差特性等角度展开比较，为后续IP设计与优化提供依据。
